/* .ra file decoder v1.0 */

/* Usage: radecode file.ra >output.dat */
...
char version[]="$VER: RAdecode 1.1 (22.01.98) The Amiga RealAudio decoder, thanks to the Usenet consortium."
	       "DOS version and ansification by D. Lindauer, LADSOFT"
               "OS/2 port by Lesha Bogdanow";

void decode_block(unsigned char *source,signed short *target);
INTTYPE t_sqrt(unsigned INTTYPE x);
void do_voice(INTTYPE *a1,INTTYPE *a2);
void do_output_subblock(INTTYPE x);
void rotate_block(short *source,short *target,INTTYPE offset);
INTTYPE irms(short *data,INTTYPE factor);
void add_wav(INTTYPE n,INTTYPE f,INTTYPE m1,INTTYPE m2,INTTYPE m3,short *s1,short *s2,short *s3,short *dest);
void final(short *i1,short *i2,void *out,INTTYPE *statbuf,INTTYPE len);
void unpack_input(unsigned char *input,unsigned INTTYPE *output);
INTTYPE rms(INTTYPE *data,INTTYPE f);
void dec1(INTTYPE *data,INTTYPE *inp,INTTYPE n,INTTYPE f);
void dec2(INTTYPE *data,INTTYPE *inp,INTTYPE n,INTTYPE f,INTTYPE *inp2,INTTYPE l);
INTTYPE eq(short *in,INTTYPE *target);

/* Globals */
unsigned int resetflag; /* reset flag */
unsigned INTTYPE unpacked[28]; /* buffer for unpacked input */
unsigned INTTYPE *iptr; /* poINTTYPEer to current input (from unpacked) */
unsigned INTTYPE gval;
unsigned short *gsp;
unsigned INTTYPE gbuf1[8];
unsigned short gbuf2[120];
signed short output_buffer[40];
unsigned INTTYPE *decptr; /* decoder ptr */
short *decsp; 
unsigned INTTYPE val;
unsigned INTTYPE oldval=0;
/* the swapped buffers: */
unsigned INTTYPE *swapbuf1;
unsigned INTTYPE *swapbuf2;
unsigned INTTYPE *swapbuf1alt;
unsigned INTTYPE *swapbuf2alt;
unsigned INTTYPE swapb1a[10];
unsigned INTTYPE swapb2a[10];
unsigned INTTYPE swapb1b[10];
unsigned INTTYPE swapb2b[10];
/* consts */
...
void ra144(FILE *fdo,FILE *fd,FILE *fdl)
{
  ...
  out=malloc(160*sizeof(short)*BLOCK_SIZE);
  if (!out) return;
  if (!fdo) if (dart_open()) return;

  fread(input,2,1,fd);
  while(input[0])
  fread(input,256,1,fd);
  fread(input,input[1],1,fd);
  resetflag=1;
  val=oldval=0;
  swapbuf1=swapb1a;
  swapbuf2=swapb2a;
  ...
  swapbuf2alt=swapb2b;
	...
	printf("decoding:  ");
  while(fread(input,20,1,fd)>0)
  {
    decode_block(input,out+cc*160);
    cc++;
    total++;
    if (cc==BLOCK_SIZE) {
       fprintf(fdl,"\rdecoding: %u:%02u",(total/50)/60,(total/50)%60);
       if (fdo) fwrite(out,320*cc,1,fdo);
       else dart_put(out,160*cc);
       cc=0;
       }
  }
...
if (!fdo) {
   fprintf(fdl,"\rdecoded: %u:%02u, waiting for playback to finish",
           (total/50)/60,(total/50)%60);
   dart_flush();
   dart_close();
   }
}
...
/* data tables */
unsigned short sqrt_table[4096]={
...
signed short wavtable1_144[2304]={
...
unsigned short wavtable2_144[2304]={
...
static short decodetable1_144[63]={
...
static short decodetable2_144[127]={
...
void decode_block(unsigned char *source,signed short *target)
{
  ...
  do_voice(swapbuf1,swapbuf2);

  a=t_sqrt(val*oldval)>>12;

  for(c=0;c<NBLOCKS;c++) {
    if(c==(NBLOCKS-1)) {
      dec1(swapbuf1,swapbuf2,3,val);
    }else{
      if(c*2==(NBLOCKS-2)) {
        if(oldval<val) {
          dec2(swapbuf1,swapbuf2,3,a,swapbuf2alt,c);
        }else{
          dec2(swapbuf1alt,swapbuf2alt,3,a,swapbuf2,c);
        }
      }else{
        if(c*2<(NBLOCKS-2)) {
          dec2(swapbuf1alt,swapbuf2alt,3,oldval,swapbuf2,c);
        }else{
          dec2(swapbuf1,swapbuf2,3,val,swapbuf2alt,c);
        }
      }
    }
  }
  ...
  oldval=val;
  temp=swapbuf1alt;
  swapbuf1alt=swapbuf1;
  swapbuf1=temp;
  temp=swapbuf2alt;
  swapbuf2alt=swapbuf2;
  swapbuf2=temp;
}
...
INTTYPE t_sqrt(unsigned INTTYPE x)
{
  ...
  return (((INTTYPE)sqrt_table[x])<<s)<<2;
}
...
void do_output_subblock(INTTYPE x) {
  INTTYPE a,b,c,d,e,f,g;
  static unsigned INTTYPE buffer[5];
  static unsigned short buffer2[148];
  static unsigned short buffer_a[40];
  static unsigned short buffer_b[40];
  static unsigned short buffer_c[40];
  static unsigned short buffer_d[40];

  if(x==1) memset(buffer,0,20);
  if((*iptr)==0) a=0;
  else a=(*iptr)+HALFBLOCK-1;
  iptr++;
  b=*(iptr++);
  c=*(iptr++);
  d=*(iptr++);
  if(a) rotate_block(buffer2,buffer_a,a);
  memcpy(buffer_b,etable1_144+b*BLOCKSIZE,BLOCKSIZE*2);
  e=((table1_144[b]>>4)*gval)>>8;
  memcpy(buffer_c,etable2_144+c*BLOCKSIZE,BLOCKSIZE*2);
  f=((table2_144[c]>>4)*gval)>>8;
  if(a) g=irms(buffer_a,gval)>>12;
  else g=0;
  add_wav(d,a,g,e,f,buffer_a,buffer_b,buffer_c,buffer_d);
  memmove(buffer2,buffer2+BLOCKSIZE,(BUFFERSIZE-BLOCKSIZE)*2);
  memcpy(buffer2+BUFFERSIZE-BLOCKSIZE,buffer_d,BLOCKSIZE*2);
  final(gsp,buffer_d,output_buffer,buffer,BLOCKSIZE);
}
...
{
  short *p1,*p2;
  unsigned INTTYPE sum;
  p2=(p1=data)+BLOCKSIZE;
  for(sum=0;p2>p1;p1++) sum+=((INTTYPE)(*p1))*(*p1);
...
void add_wav(INTTYPE n,INTTYPE f,INTTYPE m1,INTTYPE m2,INTTYPE m3,short *s1,short *s2,short *s3,short *dest)
{
  ...
  ptr=wavtable1_144+n*9;
  ptr2=wavtable2_144+n*9;
  if(f!=0) {
    a=((*ptr)*m1)>>((*ptr2)+1); 
  }else{
    a=0;
...
void unpack_input(unsigned char *input,unsigned INTTYPE *output)
{
  unsigned INTTYPE outbuffer[28];
  unsigned short inbuffer[10];
  unsigned int x;
  unsigned INTTYPE *ptr;

  /* fix endianness */
  for(x=0;x<20;x+=2)
  {
    inbuffer[x/2]=(((short)input[x])<<8)+input[x+1];
  }
  ...
  *(output++)=outbuffer[11];
  for(x=1;x<11;x++) *(output++)=outbuffer[x];
  ptr=outbuffer+12;
  for(x=0;x<16;x+=4)
  {
    *(output++)=ptr[x];
...
void dec2(INTTYPE *data,INTTYPE *inp,INTTYPE n,INTTYPE f,INTTYPE *inp2,INTTYPE l)
{
  static unsigned short *sptr;
  unsigned INTTYPE *ptr1,*ptr2;
  INTTYPE work[10];
  INTTYPE a,b;
  int x;
  INTTYPE result;
...
INTTYPE eq(short *in,INTTYPE *target)
{
  ...
  unsigned INTTYPE u;
  short *sptr;
  INTTYPE *ptr1,*ptr2,*ptr3;
  INTTYPE *bp1,*bp2,*temp;
  static INTTYPE buffer1[10];
  static INTTYPE buffer2[10];
  ...
  a=bp2[9];
  *target=a;
  if(a+0x1000>0x1fff) exit(1); /* oops */
  c=8;u=a;
  while(c>=0)
  {
    if(u==0x1000) u++;
